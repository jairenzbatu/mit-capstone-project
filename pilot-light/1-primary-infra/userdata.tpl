...
aws s3 ls "$BUCKET/database-dump/"
LATEST_FILE=$(aws s3 ls "$BUCKET/database-dump/" | awk '{print $4}' | grep '\.zip$' | sort -r | head -n 1)

# Check if a file was found
if [[ -z "$LATEST_FILE" ]]; then
    echo "No .zip files found in $BUCKET/database-dump"
elif [[ -n "$LATEST_FILE" ]]; then
    # Download the latest file
    echo "Downloading latest backup: $LATEST_FILE"
    aws s3 cp $BUCKET/database-dump/$LATEST_FILE "$DABATABSE_DIR/"
...
# Store the PHP code in a variable
PHP_CODE='
<?php
...
-v /opt/info.php:/var/www/html/public/info.php \
snipe/snipe-it
